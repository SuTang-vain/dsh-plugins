// dsh-prior-probe — official bundle entry (ESM plugin module).
// Loaded by the row in cordis.patch.yml; registers three model tools through
// the tools registry. Batteries are embedded, so the plugin works with the
// data/ directory deleted; those files ship only for provenance audit.
...
export const name = 'dsh-prior-probe'
export const inject = ['tools']

export function apply(ctx) {
  console.log('[dsh-prior-probe] bundle loaded')
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args) {
      ...
      const perProbe = battery.probes.map(function (probe) {
        ...
        return { id: probe.id, pass: result.pass, flag, sample: text.slice(0, 120) + (text.length > 120 ? '...' : '') }
      })
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args) {
      ...
      async function oneShot(probe) {
        ...
        let result = null
        try {
          result = await web.fetch({ url, method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: 'Bearer ' + apiKey }, body: JSON.stringify(payload) })
        } catch (error) {
      ...
      for (const probe of battery.probes) {
        ...
        const row = { id: probe.id, target: probe.target || battery.target, expected_direction: probe.expected_direction, attempts: transportError ? 0 : repeats, passes, G: transportError ? null : passes / repeats, samples }
