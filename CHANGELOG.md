...
- **utility evidence**: a deterministic offline demo for prior-probe
  generated by the shipped scoring code
  (`data/example-score-run.json`, shown in the plugin README); an example
  version-tree dataset built from the real npm publish history of
  `@deepseek-ai/dsh` (`data/examples/versions-dsh-official.json`); and the
